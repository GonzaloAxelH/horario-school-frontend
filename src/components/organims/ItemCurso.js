...
right: 8%;
bottom: 8px;
transform: translateX(-50%) rotate(${(props) => props.large ? '180deg' : '0deg'});
transition: 0.5s all;
...
const ItemCurso = ({ color, title, docente, participants, warning, infomation, contraido }) => {
    ...
    return (
        <ItemWrapper fondo={color} large={toggleInformation}>
            <TitleCurse contraido={contraido} to={`/curso/${title}`}>
                {title}
            </TitleCurse>
            <NameDocente contraido={contraido}>
                {docente}
            </NameDocente>
            <Participants contraido={contraido}> <b>{participants}</b> Participantes</Participants>
            <ArrowImg contraido={contraido} src={arrowImg} alt="image" />
            <WarningInfo contraido={contraido} >
                {warning ? <Exclamation src={exclamationImg} alt="Asa" /> : <Exclamation src={checkImg} alt="Asa" />}
                {warning ? <Info>Tareas pendientes </Info> : <Info>Tareas Completadas</Info>}
            </WarningInfo>
            <ArrowDown contraido={contraido} onClick={openInformation} large={toggleInformation} src={arrowdowImg} alt="As" />
